...
class ConsolidatorErrorHandler:
    ...
    def handle_file_system_error(
        self, file_path: Path, error: Exception, operation: str = "file operation"
    ) -> bool:
        """
    ...
    def handle_json_parsing_error(
        ...
        if hasattr(error, "lineno") and hasattr(error, "colno"):
            ...
            self.logger.error(
                f"JSON syntax error in {file_path} at line {error.lineno}, "  # type: ignore
                f"column {error.colno}: {str(error)}"  # type: ignore
            )
        else:
    ...
    def handle_validation_error(
        self, file_path: Path, error: Exception, field_name: Optional[str] = None
    ) -> bool:
        """
    ...
    def handle_memory_error(
        ...
        self.logger.info(
            "Consider processing files in smaller batches or increasing available memory"
        )
    ...
    def handle_directory_error(self, directory_path: Path, error: Exception) -> bool:
        ...
        if isinstance(error, FileNotFoundError):
            ...
            self.logger.info(
                "Please ensure the reports directory exists and contains YData Profiling JSON files"
            )
            self._add_error(
                "directory_not_found",
                directory_path,
    ...
    def handle_empty_directory(self, directory_path: Path) -> None:
        ...
        self.logger.info(
            "Please ensure the directory contains YData Profiling JSON reports"
        )
        self.warnings.append(warning_msg)

    def log_parsing_warning(
        self, file_path: Path, field: str, default_value: any, reason: str = ""  # type: ignore
    ) -> None:
        """
        ...
        warning_msg = (
            f"Using default value for field '{field}' in {file_path.name}: "
    ...
    def handle_unexpected_error(
        self, error: Exception, context: str = "", file_path: Optional[Path] = None
    ) -> bool:
        """
    ...
    def create_error_summary(self) -> str:
        ...
        if self.errors:
            summary_lines.extend(
                [f"", f"### Errors Encountered ({len(self.errors)} total)", ""]
            )
            ...
            for error_type, error_list in error_by_type.items():
                summary_lines.append(
                    f"**{error_type.replace('_', ' ').title()}** ({len(error_list)} errors):"
                )
        ...
        if self.warnings:
            summary_lines.extend([f"### Warnings ({len(self.warnings)} total)", ""])
        ...
        if any(error.error_type == "permission_error" for error in self.errors):
            summary_lines.append(
                "- Check file and directory permissions for the reports directory"
            )

        if any(error.error_type == "json_parsing_error" for error in self.errors):
            summary_lines.append(
                "- Verify that JSON files are valid YData Profiling reports"
            )
        ...
        if not any(
            error.error_type
            in ["permission_error", "json_parsing_error", "memory_error"]
            for error in self.errors
        ):
            summary_lines.append(
                "- Review the detailed error messages above for specific guidance"
            )

...
class ReportGenerator:
    ...
    def generate_consolidated_report(
        ...
        priority_issues = self._format_quality_issues(quality_issues, self.max_priority_issues)
        ...
        main_report = self._apply_length_limits(main_report, self.max_main_report_length)
        ...
        critical_issues_count = len([issue for issue in quality_issues if issue.severity == 'critical'])
    ...
    def _create_executive_summary(self, analysis: DatasetAnalysis) -> str:
        ...
        # Basic metrics
        summary_parts.append(f"**Total Datasets Analyzed:** {analysis.total_datasets}")
        summary_parts.append(f"**Analysis Date:** {analysis.analysis_timestamp.strftime('%Y-%m-%d %H:%M')}")
        
        # Size distribution
        size_dist = analysis.size_distribution
        if size_dist:
            summary_parts.append(f"**Dataset Size Distribution:**")
            summary_parts.append(f"- Small datasets (< 1MB): {size_dist.small_datasets}")
            summary_parts.append(f"- Medium datasets (1-100MB): {size_dist.medium_datasets}")
            summary_parts.append(f"- Large datasets (> 100MB): {size_dist.large_datasets}")
            summary_parts.append(f"- Total memory usage: {self._format_memory_size(size_dist.total_memory_usage)}")
        else:
            summary_parts.append(f"**Dataset Size Distribution:** No size information available")
        
        # Quality metrics
        quality = analysis.quality_summary
        if quality:
            summary_parts.append(f"**Data Quality Overview:**")
            summary_parts.append(f"- Datasets with quality issues: {quality.datasets_with_issues}")
            summary_parts.append(f"- High priority issues: {quality.high_priority_issues}")
            summary_parts.append(f"- Overall quality score: {quality.overall_quality_score:.1f}/100")
        else:
            summary_parts.append(f"**Data Quality Overview:** No quality information available")
        
        # Category overview
        if analysis.category_breakdown:
            summary_parts.append(f"**Dataset Categories:** {len(analysis.category_breakdown)} categories identified")
            top_categories = sorted(analysis.category_breakdown.items(), key=lambda x: x[1], reverse=True)[:3]
            for category, count in top_categories:
                summary_parts.append(f"- {category}: {count} datasets")
        
        # Schema insights
        schema = analysis.schema_analysis
        if schema:
            summary_parts.append(f"**Schema Overview:**")
            summary_parts.append(f"- Unique column names: {len(schema.unique_column_names)}")
            summary_parts.append(f"- Common data types: {len(schema.common_data_types)}")
            summary_parts.append(f"- Schema inconsistencies: {len(schema.schema_inconsistencies)}")
        else:
            summary_parts.append(f"**Schema Overview:** No schema information available")
    ...
    def _format_schema_overview(self, schema_analysis: SchemaAnalysis) -> str:
        ...
        if schema_analysis.common_data_types:
            type_items = sorted(schema_analysis.common_data_types.items(), key=lambda x: x[1], reverse=True)
            for data_type, count in type_items[:10]:  # Top 10 most common
                sections.append(f"- **{data_type}**: {count} datasets")
        else:
        ...
        if schema_analysis.most_common_columns:
            for column_name, count in schema_analysis.most_common_columns[:10]:
                sections.append(f"- **{column_name}**: appears in {count} datasets")
        else:
        ...
        if schema_analysis.data_type_distribution_by_category:
            for category, type_dist in schema_analysis.data_type_distribution_by_category.items():
                sections.append(f"\n**{category}:**")
                sorted_types = sorted(type_dist.items(), key=lambda x: x[1], reverse=True)
                for data_type, count in sorted_types[:5]:  # Top 5 per category
                    sections.append(f"- {data_type}: {count}")
    ...
    def _generate_category_breakdown(self, analysis: DatasetAnalysis) -> str:
        ...
        # Category summary
        sections.append(f"**{len(analysis.category_breakdown)} categories identified:**")
        ...
        for category, count in sorted_categories:
            ...
            if category in analysis.category_insights:
                ...
                if insight.common_patterns:
                    ...
                    for pattern in insight.common_patterns[:3]:  # Top 3 patterns
                        sections.append(f"- {pattern}")
                
                if insight.quality_trends:
    ...
    def _format_quality_issues(self, issues: List[QualityIssue], max_issues: int = 10) -> str:
        ...
        sorted_issues = sorted(
            issues, 
            key=lambda x: (severity_order.get(x.severity, 4), x.dataset_name)
        )
        ...
        sections = []
        sections.append(f"**{len(display_issues)} highest priority issues** (of {len(issues)} total):")
        
        for i, issue in enumerate(display_issues, 1):
            sections.append(f"\n**{i}. {issue.dataset_name}** - {issue.severity.upper()}")
            sections.append(f"- **Issue:** {issue.description}")
            sections.append(f"- **Recommendation:** {issue.recommendation}")
            
            if issue.affected_columns:
                columns_str = ", ".join(issue.affected_columns[:5])  # Limit column list
                if len(issue.affected_columns) > 5:
                    columns_str += f" (and {len(issue.affected_columns) - 5} more)"
                sections.append(f"- **Affected columns:** {columns_str}")
            ...
            if issue.metrics:
                ...
                for metric, value in issue.metrics.items():
                    if isinstance(value, float):
                        key_metrics.append(f"{metric}: {value:.1f}%")
                    else:
                        key_metrics.append(f"{metric}: {value}")
                if key_metrics:
                    sections.append(f"- **Metrics:** {', '.join(key_metrics)}")
    ...
    def _create_recommendations_section(
        ...
        if analysis.standardization_opportunities:
            ...
            for i, opp in enumerate(analysis.standardization_opportunities[:5], 1):
                ...
                sections.append(f"- **Description:** {opp.description}")
                sections.append(f"- **Potential Benefit:** {opp.potential_benefit}")
                sections.append(f"- **Affected Datasets:** {len(opp.affected_datasets)} datasets")
        
        # Schema recommendations
        if analysis.schema_analysis and analysis.schema_analysis.schema_recommendations:
            sections.append("\n### Schema Improvement Recommendations")
            high_priority_recs = [r for r in analysis.schema_analysis.schema_recommendations if r.priority == 'high']
            
            for i, rec in enumerate(high_priority_recs[:3], 1):  # Top 3 high priority
                ...
                sections.append(f"- **Affected Datasets:** {len(rec.affected_datasets)} datasets")
        
        # Quality-based recommendations
        critical_issues = [issue for issue in quality_issues if issue.severity in ['critical', 'high']]
        if critical_issues:
            ...
            for issue_type, group_issues in issue_groups.items():
                ...
                for issue in group_issues[:3]:  # Top 3 per type
                    sections.append(f"- {issue.dataset_name}: {issue.recommendation}")
                    # Add "recommend" or "suggest" if not already present
                    if "recommend" not in issue.recommendation.lower() and "suggest" not in issue.recommendation.lower():
                        sections.append(f"  Recommended action: {issue.recommendation}")
        
        if not sections:
            sections.append("No specific recommendations identified. All datasets appear to be in good condition.")
        
        return "\n".join(sections)
    
    def _format_main_report(self, sections: List[Tuple[str, str]], analysis: DatasetAnalysis) -> str:
        ...
        if self.config.include_detailed_appendix:
            report_parts.append("See detailed appendix for comprehensive dataset information.*")
        else:
            report_parts.append("Enable detailed appendix in configuration for comprehensive dataset information.*")
    ...
    def _create_detailed_appendix(
        ...
        sections.append("")
        sections.append("This appendix contains comprehensive details for all analyzed datasets.")
        ...
        if quality_issues:
            for issue in quality_issues:
                ...
                if issue.affected_columns:
                    sections.append(f"**Affected Columns:** {', '.join(issue.affected_columns)}")
        ...
        sections.append("\n### Column Naming Analysis")
        if schema.column_naming_analysis and schema.column_naming_analysis.naming_conventions:
            sections.append("**Naming Conventions Detected:**")
            for convention, examples in schema.column_naming_analysis.naming_conventions.items():
                sections.append(f"- **{convention}:** {', '.join(examples[:5])}")
        
        if schema.column_naming_analysis and schema.column_naming_analysis.inconsistent_naming:
            sections.append("\n**Inconsistent Naming Patterns:**")
            for pattern, variations in schema.column_naming_analysis.inconsistent_naming:
        ...
        if schema.schema_inconsistencies:
            ...
            for inconsistency in schema.schema_inconsistencies:
                ...
                sections.append(f"- **Affected Datasets:** {', '.join(inconsistency.affected_datasets)}")
        ...
        if schema.schema_recommendations:
            ...
            for rec in schema.schema_recommendations:
                ...
                sections.append(f"- **Description:** {rec.description}")
                sections.append(f"- **Implementation:** {rec.implementation_notes}")
                sections.append(f"- **Affected Datasets:** {', '.join(rec.affected_datasets)}")
    ...
    def _apply_length_limits(self, content: str, max_length: int) -> str:
        ...
        if len(content) > len('\n'.join(truncated_lines)):
            ...
            if current_length + len(truncation_notice) <= max_length + 100:  # Small buffer for notice
    ...
    def format_quality_issues(self, issues: List[QualityIssue], max_issues: int = 10) -> str:
        """
        Format prioritized quality issues section.
...
*This report focuses on the most critical insights.*"""
    ...
    def generate_report_with_template(
        ...
        template_vars = {
            ...
            'priority_issues': self._format_quality_issues(quality_issues, self.max_priority_issues),
            'recommendations': self._create_recommendations_section(analysis, quality_issues),
            'common_data_types': self._format_common_data_types(analysis.schema_analysis),
            'schema_patterns_by_category': self._format_schema_patterns_by_category(analysis),
            'column_standardization': self._format_column_standardization_analysis(analysis.schema_analysis.column_naming_analysis),
            ...
            'schema_recommendations': self._format_schema_recommendations(analysis.schema_analysis.schema_recommendations),
            'data_type_issues': self._format_data_type_consistency_issues(analysis.schema_analysis.schema_inconsistencies),
        ...
        # Apply length limits
        formatted_report = self._apply_length_limits(formatted_report, self.max_main_report_length)
    ...
    def _format_common_data_types(self, schema_analysis: SchemaAnalysis) -> str:
        ...
        lines = []
        sorted_types = sorted(schema_analysis.common_data_types.items(), key=lambda x: x[1], reverse=True)
    ...
    def _format_schema_patterns_by_category(self, analysis: DatasetAnalysis) -> str:
        ...
        lines = []
        for category, type_dist in analysis.schema_analysis.data_type_distribution_by_category.items():
            lines.append(f"\n**{category}:**")
            sorted_types = sorted(type_dist.items(), key=lambda x: x[1], reverse=True)
    ...
    def _format_column_standardization_analysis(self, column_analysis: ColumnNamingAnalysis) -> str:
    ...
    def _format_schema_patterns(self, schema_patterns: List[SchemaPattern]) -> str:
        ...
        for pattern in schema_patterns:
            ...
            lines.append(f"- **Common Columns:** {', '.join(pattern.common_columns[:5])}")
    ...
    def _format_data_type_consistency_issues(self, inconsistencies: List[SchemaInconsistency]) -> str:
        """Format data type consistency issues for template."""
        if not inconsistencies:
            return "No significant data type consistency issues detected."
        
        lines = []
        type_issues = [inc for inc in inconsistencies if inc.inconsistency_type == 'data_type_mismatch']
        
        for issue in type_issues[:5]:  # Top 5 issues
            lines.append(f"\n**{issue.column_details.get('column_name', 'Unknown Column')}**")
            lines.append(f"- **Issue:** {issue.description}")
            lines.append(f"- **Severity:** {issue.severity}")
            lines.append(f"- **Affected Datasets:** {len(issue.affected_datasets)} datasets")
    ...
    def _create_detailed_schema_section(self, analysis: DatasetAnalysis) -> str:
        ...
        if schema.data_type_distribution_by_category:
            ...
            for category, type_dist in schema.data_type_distribution_by_category.items():
                lines.append(f"\n*{category}:*")
                sorted_types = sorted(type_dist.items(), key=lambda x: x[1], reverse=True)

...
class ReportScanner:
    ...
    def scan_reports_directory(self, reports_path: Path) -> ScanResult:
        ...
        try:
            if not reports_path.exists():
                self.error_handler.handle_directory_error(reports_path, FileNotFoundError(f"Directory not found: {reports_path}"))
                return ScanResult(
                    valid_reports=[],
                    invalid_files=[],
            ...
            if not reports_path.is_dir():
                self.error_handler.handle_directory_error(reports_path, NotADirectoryError(f"Path is not a directory: {reports_path}"))
                return ScanResult(
                    valid_reports=[],
                    invalid_files=[],
                    total_files_scanned=0,
        ...
        try:
            ...
            self.error_handler.handle_file_system_error(reports_path, e, "directory scanning")
            return ScanResult(
                valid_reports=[],
                invalid_files=[],
                total_files_scanned=0,
            ...
            self.error_handler.handle_unexpected_error(e, "directory scanning", reports_path)
            return ScanResult(
                valid_reports=[],
                invalid_files=[],
                total_files_scanned=0,
        ...
        for i, json_file in enumerate(json_files):
            if self.progress_callback:
                self.progress_callback(i + 1, total_files, f"Processing {json_file.name}")
            ...
            try:
                ...
                if self.error_handler.handle_file_system_error(json_file, e, "file processing"):
                    invalid_files.append(json_file)
                else:
                    # Critical error - might need to stop processing
                    logger.critical(f"Critical file system error processing {json_file}: {e}")
                    break
                    
            except json.JSONDecodeError as e:
                # Handle JSON parsing errors
                self.error_handler.handle_json_parsing_error(json_file, e, "structure validation")
                invalid_files.append(json_file)
                
            except MemoryError as e:
                # Handle memory errors
                if not self.error_handler.handle_memory_error(json_file, e, "file processing"):
                    logger.critical("Memory error during scanning - stopping processing")
                    break
                    
            except Exception as e:
                # Handle unexpected errors
                if not self.error_handler.handle_unexpected_error(e, "file processing", json_file):
                    logger.critical(f"Critical unexpected error processing {json_file}: {e}")
        ...
        if self.error_handler.errors:
            logger.warning(f"Encountered {len(self.error_handler.errors)} errors during scanning")
    ...
    def validate_ydata_profiling_structure(self, file_path: Path) -> bool:
        ...
        try:
            ...
            if not all(key in data for key in required_keys):
                logger.debug(f"Missing required top-level keys in {file_path}: "
            ...
            if not isinstance(analysis, dict):
                self.error_handler.log_parsing_warning(file_path, 'analysis', 'dict', 
            ...
            if missing_analysis:
                self.error_handler.log_parsing_warning(file_path, 'analysis', 'required fields', 
            ...
            if not isinstance(table, dict):
                self.error_handler.log_parsing_warning(file_path, 'table', 'dict', 
                                                     f"Expected dict, got {type(table)}")
                return False
            
            table_required = ['n', 'n_var', 'memory_size']
            missing_table = [key for key in table_required if key not in table]
            if missing_table:
                self.error_handler.log_parsing_warning(file_path, 'table', 'required fields', 
            ...
            if not isinstance(variables, dict):
                self.error_handler.log_parsing_warning(file_path, 'variables', 'dict', 
            ...
            if variables:
                ...
                if not isinstance(first_var, dict):
                    self.error_handler.log_parsing_warning(file_path, f'variables.{first_var_name}', 'dict', 
                ...
                if missing_var:
                    self.error_handler.log_parsing_warning(file_path, f'variables.{first_var_name}', 'required fields', 
            ...
            self.error_handler.handle_validation_error(file_path, e, "structure validation")
            return False
        except Exception as e:
            self.error_handler.handle_unexpected_error(e, "structure validation", file_path)
            return False
    ...
    def _create_report_file(self, file_path: Path) -> ReportFile:
        ...
        try:
            ...
            try:
                ...
                self.error_handler.handle_file_system_error(file_path, e, "file metadata extraction")
                # Use default values
                file_size = 0
            ...
            self.error_handler.handle_unexpected_error(e, "ReportFile creation", file_path)
    ...
    def _extract_dataset_name(self, file_path: Path) -> str:
        ...
        try:
            ...
            self.error_handler.log_parsing_warning(file_path, 'dataset_name', file_path.stem, 
    ...
    def extract_dataset_metadata(self, file_path: Path) -> ReportFile:
        ...
        try:
            ...
            self.error_handler.handle_unexpected_error(e, "metadata extraction", file_path)
            return ReportFile(
                path=file_path,
                dataset_name=file_path.stem,
                category='uncategorized',
    ...
    def get_scan_summary(self, scan_result: ScanResult) -> str:
        ...
        if scan_result.valid_reports:
            ...
            original_categories = set(report.category for report in scan_result.valid_reports[:10])

...
class ProfileParser:
    ...
    def parse_profile_json(self, file_path: Path) -> ParseResult:
        ...
        try:
            ...
            try:
                profile_data = self._extract_full_profile(
                    json_data, file_path, dataset_name
                )
                return ParseResult(
                    success=True,
                ...
                self.error_handler.handle_validation_error(
                    file_path, e, "full profile extraction"
                )
                ...
                try:
                    partial_data = self._extract_partial_profile(
                        json_data, file_path, dataset_name, str(e)
                    )
                    ...
                    self.error_handler.handle_unexpected_error(
                        partial_e, "partial profile extraction", file_path
                    )
                    return ParseResult(
                        success=False,
                        profile_data=None,
                        ...
                        errors=[
                            str(e),
                            f"Partial extraction also failed: {str(partial_e)}",
                        ],
                        warnings=warnings,
                    )
                ...
                self.error_handler.handle_unexpected_error(
                    e, "profile extraction", file_path
                )
                ...
                try:
                    ...
                    return ParseResult(
                        ...
                        errors=[
                            ...
                            f"Partial extraction failed: {str(partial_e)}",
                        ],
                        warnings=warnings,
                    )
    ...
    def _extract_full_profile(
        self, json_data: dict, file_path: Path, dataset_name: str
    ) -> ProfileData:
        """
    ...
    def _extract_partial_profile(
        self, json_data: dict, file_path: Path, dataset_name: str, error_msg: str
    ) -> PartialProfileData:
        """
        ...
        try:
            if "table" in json_data:
                extracted_fields["table"] = self._extract_with_fallbacks(
                    json_data, "table"
                )
        except Exception as e:
    ...
    def _create_profile_from_partial(
        self, partial_data: PartialProfileData, file_path: Path, dataset_name: str
    ) -> ProfileData:
    ...
    def _extract_analysis_info(self, json_data: dict) -> AnalysisInfo:
        ...
        return AnalysisInfo(title=title, date_start=date_start, date_end=date_end)
    ...
    def _extract_table_stats(self, json_data: dict) -> TableStats:
        ...
        try:
            ...
            logger.debug(
                f"Error calculating duplicate_rows_pct: {e}, using default 0.0"
            )
        ...
        if not isinstance(data_types, dict):
            logger.debug(
                f"Invalid data_types format: {type(data_types)}, using empty dict"
            )
            data_types = {}
    ...
    def _extract_variable_stats(self, json_data: dict) -> List[VariableStats]:
        ...
        if not isinstance(variables_data, dict):
            logger.debug(
                f"Invalid variables format: {type(variables_data)}, using empty dict"
            )
            return variables
        ...
        for var_name, var_data in variables_data.items():
            try:
                if not isinstance(var_data, dict):
                    logger.debug(
                        f"Invalid variable data for {var_name}: {type(var_data)}, skipping"
                    )
                    continue
                ...
                try:
                    ...
                    logger.debug(
                        f"Error calculating missing_pct for {var_name}: {e}, using 0.0"
                    )
                    missing_pct = 0.0
                ...
                try:
                    ...
                    logger.debug(
                        f"Error extracting unique_values for {var_name}: {e}, using 0"
                    )
                    unique_values = 0
                ...
                try:
                    ...
                    logger.debug(
                        f"Error extracting memory_size for {var_name}: {e}, using 0"
                    )
                    memory_size = 0
            ...
            for data_type, count in types.items():
                ...
                if proportion > 0.8 and total_vars > 5:
                    quality_flags.append(
                        f"Unusual type distribution: {proportion:.1%} {data_type}"
                    )

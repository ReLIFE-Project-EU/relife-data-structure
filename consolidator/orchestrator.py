...
from .error_handler import setup_error_logging
from .generator import ReportGenerator
from .models import ConsolidationResult, ConsolidatorConfig, ProcessingError
...
class ConsolidationOrchestrator:
        ...
        try:
            ...
            try:
                ...
                return self._create_failure_result(
                    f"Configuration validation failed: {str(e)}", timestamp
                )
            ...
            with Progress(
                ...
                scan_task = progress.add_task(
                    "Scanning reports directory...", total=100
                )
                try:
                    scan_result = self.scanner.scan_reports_directory(
                    ...
                    if not scan_result.valid_reports:
                        ...
                        if scan_result.total_files_scanned == 0:
                            ...
                            try:
                                ...
                                output_path = self._write_report_to_file(
                                    consolidated_report, timestamp
                                )

                                execution_time = time.time() - self.start_time
                                self._display_success_summary(
                                    output_path, 0, execution_time
                                )
                                ...
                                return ConsolidationResult(
                                    ...
                                    warnings=self.error_handler.warnings
                                    + self.warnings,
                    ...
                    return self._create_failure_result(
                        f"Directory scanning failed: {str(e)}", timestamp
                    )

                # Step 2: Parse JSON files with comprehensive error handling
                parse_task = progress.add_task(
                    "Parsing JSON reports...", total=len(scan_result.valid_reports)
                )
                parsed_datasets = []
                parsing_errors = 0
                    ...
                    try:
                        ...
                        if parse_result.success and parse_result.profile_data:
                            ...
                            self._handle_parsing_error(
                                report_file.path, parse_result.errors
                            )

                            # If we have partial data, log it for potential future use
                            if parse_result.partial_data:
                                logger.debug(
                                    f"Partial data available for {report_file.path}"
                                )

                    except MemoryError as e:
                        self.error_handler.handle_memory_error(
                            report_file.path, e, "JSON parsing"
                        )
                        logger.critical(
                            "Memory error during parsing - stopping processing"
                        )
                        break

                    except Exception as e:
                        self.error_handler.handle_unexpected_error(
                            e, "JSON parsing", report_file.path
                        )
                        parsing_errors += 1
                ...
                logger.info(
                    f"Successfully parsed {len(parsed_datasets)} datasets, "
                    f"{parsing_errors} parsing errors"
                )

                # Step 3: Analyze datasets with error handling
                analysis_task = progress.add_task(
                    "Analyzing dataset collection...", total=100
                )
                try:
                    dataset_analysis = self.analyzer.analyze_dataset_collection(
                        parsed_datasets
                    )
                ...
                try:
                    ...
                    return self._create_failure_result(
                        f"Quality assessment failed: {str(e)}", timestamp
                    )

                # Step 5: Generate report with error handling
                report_task = progress.add_task(
                    "Generating consolidated report...", total=100
                )
                try:
                    consolidated_report = (
                        self.report_generator.generate_consolidated_report(
                    ...
                    return self._create_failure_result(
                        f"Report generation failed: {str(e)}", timestamp
                    )
                ...
                try:
                    ...
                    self.error_handler.handle_file_system_error(
                        self.config.output_file, e, "report writing"
                    )
                    return self._create_failure_result(
                        f"Failed to write report: {str(e)}", timestamp
                    )

            # Display success summary with error information
            execution_time = time.time() - self.start_time
            self._display_success_summary(
                output_path, len(parsed_datasets), execution_time
            )

            # Collect all errors from error handler
            all_errors = [error.message for error in self.error_handler.errors] + [
    ...
    def _validate_configuration(self) -> None:
        ...
        try:
            if not self.config.reports_directory.exists():
                ...
                self.error_handler.handle_directory_error(
                    self.config.reports_directory,
                    FileNotFoundError(
                        f"Directory not found: {self.config.reports_directory}"
                    ),
                )
                raise FileNotFoundError(
                    f"Reports directory not found: {self.config.reports_directory}"
                )

            if not self.config.reports_directory.is_dir():
                raise NotADirectoryError(
                    f"Reports path is not a directory: {self.config.reports_directory}"
                )
        ...
        try:
            ...
            self.error_handler.handle_file_system_error(
                self.config.output_file.parent, e, "output directory creation"
            )
            raise
        ...
        try:
            ...
            self.error_handler.handle_file_system_error(
                self.config.output_file.parent, e, "output directory write test"
            )
            raise PermissionError(
                f"Cannot write to output directory: {self.config.output_file.parent}"
            )
    ...
    def _display_startup_banner(self) -> None:
        ...
        banner_text.append(
            f"Reports Directory: {self.config.reports_directory}\n", style="dim"
        )
        banner_text.append(f"Output File: {self.config.output_file}\n", style="dim")
        banner_text.append(
            f"Timestamp: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", style="dim"
        )
    ...
    def _report_scan_results(self, scan_result) -> None:
        ...
        if scan_result.invalid_files:
            self.console.print(
                f"⚠️  Skipped {len(scan_result.invalid_files)} invalid files",
                style="yellow",
            )
    ...
    def _write_report_to_file(self, consolidated_report, timestamp: datetime) -> Path:
        ...
        try:
            with open(output_path, "w", encoding="utf-8") as f:
                ...
                if (
                    self.config.include_detailed_appendix
                    and consolidated_report.detailed_appendix
                ):
                    f.write("\n\n---\n\n")
            ...
            self.error_handler.handle_file_system_error(
                output_path, e, "report file writing"
            )
            raise
        except Exception as e:
            self.error_handler.handle_unexpected_error(
                e, "report file writing", output_path
            )
            raise
    ...
    def _display_success_summary(
        self, output_path: Path, datasets_count: int, execution_time: float
    ) -> None:
        ...
        if total_errors > 0:
            summary_text.append(
                f"⚠️  Errors encountered: {total_errors}\n", style="yellow"
            )
            ...
            if error_stats:
                ...
                for error_type, count in error_stats.items():
                    summary_text.append(
                        f"   • {error_type.replace('_', ' ').title()}: {count}\n",
                        style="dim",
                    )
    ...
    def _create_failure_result(
        self, error_msg: str, timestamp: datetime
    ) -> ConsolidationResult:
        ...
        all_errors = (
            [error_msg]
            + [error.message for error in self.error_handler.errors]
            + [error.message for error in self.errors]
        )
        all_warnings = self.error_handler.warnings + self.warnings

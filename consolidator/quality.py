...
class QualityAssessor:
    ...
    def __init__(self, thresholds: Optional[QualityThresholds] = None):
        ...
        logger.info(
            f"QualityAssessor initialized with thresholds: "
    ...
    def prioritize_datasets(self, issues: List[QualityIssue]) -> List[DatasetPriority]:
        ...
        for dataset_name, dataset_issues_list in dataset_issues.items():
            ...
            primary_issues = [
                issue.issue_type for issue in dataset_issues_list[:3]
            ]  # Top 3 issues
    ...
    def assess_memory_concerns(self, dataset: ProfileData) -> Optional[QualityIssue]:
        ...
        if memory_mb > self.thresholds.memory_concern_mb:
            severity = (
                "high"
                if memory_mb > self.thresholds.memory_concern_mb * 5
                else "medium"
            )
            ...
            return QualityIssue(
                ...
                metrics={
                    "memory_mb": memory_mb,
                    "threshold_mb": self.thresholds.memory_concern_mb,
                },
            )

        return None

    def detect_unusual_data_type_distributions(
        self, dataset: ProfileData
        ...
        for data_type, count in dataset.data_type_distribution.items():
            ...
            if proportion > self.thresholds.unusual_type_distribution_threshold:
                return QualityIssue(
                    ...
                    affected_columns=[],
                    metrics={"dominant_type": data_type, "proportion": proportion},  # type: ignore
                )

        return None

    def suggest_investigation_candidates(
        ...
        for dataset in datasets:
            ...
            if (
                assessment.investigation_priority == "high"
                or assessment.overall_score < 60
                or len(assessment.issues) >= 3
            ):
                candidates.append(dataset.dataset_name)

        logger.info(
            f"Identified {len(candidates)} datasets as investigation candidates"
        )
        return candidates
    ...
    def _check_missing_data(self, dataset: ProfileData) -> Optional[QualityIssue]:
        ...
        if missing_pct > self.thresholds.high_missing_data_pct:
            ...
            severity = self._determine_severity(
                missing_pct, self.thresholds.high_missing_data_pct
            )
            ...
            return QualityIssue(
                ...
                metrics={
                    ...
                    "threshold": self.thresholds.high_missing_data_pct,
                },
            )
    ...
    def _check_duplicates(self, dataset: ProfileData) -> Optional[QualityIssue]:
        ...
        if duplicate_pct > self.thresholds.high_duplicate_pct:
            severity = self._determine_severity(
                duplicate_pct, self.thresholds.high_duplicate_pct
            )
            ...
            return QualityIssue(
                ...
                metrics={
                    ...
                    "threshold": self.thresholds.high_duplicate_pct,
                },
            )
    ...
    def _calculate_quality_score(
        self, dataset: ProfileData, issues: List[QualityIssue]
    ) -> float:
    ...
    def _determine_investigation_priority(
        self, issues: List[QualityIssue], quality_score: float
    ) -> str:
    ...
    def _generate_priority_actions(self, issues: List[QualityIssue]) -> List[str]:
        ...
        for issue in issues:
            if issue.issue_type == "missing_data":
                actions.add(
                    "Investigate data collection and implement missing data handling"
                )
            elif issue.issue_type == "duplicates":
    ...
    def _get_missing_data_recommendations(self, issue: QualityIssue) -> List[str]:
        ...
        if issue.severity in ["high", "critical"]:
            recommendations.insert(
                0,
                "Urgent: High levels of missing data may indicate systematic collection issues",
            )
    ...
    def _get_duplicate_recommendations(self, issue: QualityIssue) -> List[str]:
        ...
        if issue.severity in ["high", "critical"]:
            recommendations.insert(
                0, "Urgent: High duplicate rates suggest data pipeline issues"
            )
